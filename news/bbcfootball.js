...
async function fetchBBCFootballNews() {
    const n = [];
    try {
        console.log('⚽ Fetching BBC Football news...');

        const rssRes = await axios.get('https://feeds.bbci.co.uk/sport/football/rss.xml', {
            timeout: 10000,
            headers: {
        ...
        for (const item of items.slice(0, 2)) {
            try {
                ...
                if (titleMatch?.[1] && linkMatch?.[1]) {
                    ...
                    console.log(`⚽ BBC Football: Scraping "${title.substring(0, 40)}..."`);
                    ...
                    console.log(`⚽ BBC Football: ${title.substring(0, 40)}... [${desc.length} chars]`);
                }
            } catch (e) {}
        ...
        console.error('❌ BBC Football fetch error:', e.message);
    }

    console.log(`⚽ BBC Football: ${n.length} articles`);
    return n;
}

module.exports = fetchBBCFootballNews;
